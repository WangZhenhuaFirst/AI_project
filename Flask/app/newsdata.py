...
class NewsData(object):
    ...
    def init():
        if len(NewsData.news_content_list) == 0:
            ...
            # 把新闻语料转成list
            NewsData.news_content_list = news_data['content'].fillna(
                '').to_list()
    ...
    def clean():
        # 释放
        if len(NewsData.news_content_list) > 0:
            del NewsData.news_content_list
            del NewsData.news_title_list
            gc.collect()
            NewsData.news_title_list = []
            NewsData.news_content_list = []
        pass
