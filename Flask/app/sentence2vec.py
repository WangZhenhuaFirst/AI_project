...
class sentence2vec(object):
    ...
    def clean():
        # 释放
        if sentence2vec.model is not None:
            del sentence2vec.model
            gc.collect()
            sentence2vec.model = None
        pass
    ...
    def get_plot(x1, x2, top_n):
        ...
        plt.plot(x1[:top_n], linestyle='-.', marker='o',
                 color='r', alpha=0.5, label='平滑前')
        plt.plot(x2[:top_n], linestyle='-.', marker='o',
                 color='g', alpha=0.5, label='平滑后')
    ...
    def get_sen_doc_cosine(text, title, top_n=10, plot=True):
        ...
        # 文章向量化,标题向量化
        doc_vec = sentence2vec.sif_embeddings(
            [text], sentence2vec.model, alpha=1e-3)
        # 定义句子/文章向量  和  句子/标题向量 余弦距离空字典
        sen_doc_cosine = {}
        # 遍历文章分句，计算句向量，把文章的内容和对应的余弦距离存入字典
        for sen in split_sens:
            sen_vec = sentence2vec.sif_embeddings(
                [sen], sentence2vec.model, alpha=1e-3)
            # 计算 句子/文章向量 的余弦距离
    ...
    def get_sen_doc_title_cosine(text, title, weight=0.5, top_n=10, plot=True):
        ...
        doc_vec = sentence2vec.sif_embeddings(
            [text], sentence2vec.model, alpha=1e-3)
        title_vec = sentence2vec.sif_embeddings(
            [title], sentence2vec.model, alpha=1e-3)
        # 定义句子/文章向量  和  句子/标题向量 余弦距离空字典
        ...
        knn_before_cosine_values = np.array(
            sen_doc_cosine_values) * weight + np.array(sen_title_cosine_values) * (1 - weight)
        # 使用自定义的knn_smooth函数，计算新的余弦距离 （平滑后的余弦距离）
        knn_after_cosine_values = np.array(sentence2vec.knn_smooth(
            sen_doc_cosine_values)) * weight + np.array(sentence2vec.knn_smooth(sen_title_cosine_values)) * (1 - weight)
        # 定义knn平滑后的余弦距离空字典
        knn_cosine_score = {}
        # 把原分句内容和平滑后的余弦距离组合成字典
        knn_cosine_score = dict(
    ...
    def get_summarize(text, title, weight=0.5, top_n=10, plot=False):
        ...
        if title == '':
            ranking_sentences = sentence2vec.get_sen_doc_cosine(
                text, title, top_n=top_n, plot=plot)
        else:
