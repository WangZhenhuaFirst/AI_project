...
def seg_with_jieba(infile, outfile, stopword=False):
    '''新闻语料，中文分词及去中文停用词主函数,segment the input file with jieba'''
...
def news_seg_main():
    ...
    parser.add_option('-i', '--input', dest='infile',
                      default='corpus.news.txt', help='input file to be segmented')
    # 临时存储已去掉多余字符的分词txt文件
    parser.add_option('-o', '--output', dest='outfile',
                      default='corpus.news.segwithb_temp.txt', help='output file segmented')
    # 输出处理好的分词文件，默认corpus.news.segwithb.txt
    parser.add_option('-n', '--new_output', dest='new_outfile',
                      default='05_corpus.news.segwithb.txt', help='new_output file segmented')
