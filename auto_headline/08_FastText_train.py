...
def FastText_train(infile, outmodel, outvector, size, window, min_count, sg):
    ...
    # 训练模型
    model = FastText(LineSentence(infile), size=size, window=window,
                     min_count=min_count, sg=sg, workers=multiprocessing.cpu_count())
...
def FastText_train_main():
    ...
    parser.add_option('-i', '--input', dest='infile',
                      default='06_corpus.zhwiki_news.segwithb.txt', help='zhwiki corpus')
    # 设置保存模型的参数
    parser.add_option('-m', '--outmodel', dest='wv_model',
                      default='zhwiki_news.FastText.model', help='FastText model')
    parser.add_option('-v', '--outvec', dest='wv_vectors',
                      default='zhwiki_news.FastText.vectors', help='FastText vectors')
    # 设置训练模型的参数size，是指特征向量的维度，默认为100。大的size需要更多的训练数据,但是效果会更好
    parser.add_option('-s', type='int', dest='size',
                      default=200, help='word vector size')
    # 设置训练模型的参数window，表示当前词与预测词在一个句子中的最大距离是多少,默认值为5
    parser.add_option('-w', type='int', dest='window',
                      default=5, help='window size')
    # 设置训练模型的参数min_count，可以对字典做截断. 词频少于min_count次数的单词会被丢弃掉, 默认值为5
    parser.add_option('-n', type='int', dest='min_count',
                      default=2, help='min word frequency')
    ...
    try:
        FastText_train(infile, outmodel, outvec,
                       vec_size, window, min_count, sg)
