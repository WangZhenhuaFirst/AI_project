    ...
    for s in sentences:
        ...
        for w in s:
            ...
            if w in vlookup:
                for i in range(size):
                    # 平滑逆频率，SIF
                    v[i] += (alpha / (alpha + (vlookup[w].count / Z))
                             ) * vectors[w][i]
                count += 1
        if count > 0:
            for i in range(size):
...
def get_keywords(s, topK=5):
    ...
    keywords = jieba.analyse.textrank(
        s, topK=topK, withWeight=True, allowPOS=allow_pos)
    # print(keywords)
    keywords_item = {}
...
def get_sen_doc_title_cosine(text, title, weight=0.5, top_n=10, plot=True):
    ...
    # 使用自定义的knn_smooth函数，计算新的余弦距离 （平滑后的余弦距离）
    knn_after_cosine_values = np.array(knn_smooth(
        sen_doc_cosine_values)) * weight + np.array(knn_smooth(sen_title_cosine_values)) * (1 - weight)
    # 定义knn平滑后的余弦距离空字典
    knn_cosine_score = {}
    # 把原分句内容和平滑后的余弦距离组合成字典
    knn_cosine_score = dict(zip(sen_doc_cosine_keys, knn_after_cosine_values))
...
def get_summarize(text, title, weight=0.5, top_n=10, plot=False):
    ...
    if title == '':
        ranking_sentences = get_sen_doc_cosine(
            text, title, top_n=top_n, plot=plot)
    else:
        ranking_sentences = get_sen_doc_title_cosine(
