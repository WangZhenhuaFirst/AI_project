...
def word2vec_train(infile, outmodel, outvector, size, window, min_count, sg):
    ...
    # 训练模型
    model = Word2Vec(LineSentence(infile), size=size, window=window,
...
def word2vec_train_main():
    ...
    parser.add_option('-m', '--outmodel', dest='wv_model',
                      default='zhwiki_news.word2vec.model', help='word2vec model')
    parser.add_option('-v', '--outvec', dest='wv_vectors',
                      default='zhwiki_news.word2vec.vectors', help='word2vec vectors')
    # 设置训练模型的参数size，是指特征向量的维度，默认为100。大的size需要更多的训练数据,但是效果会更好
    parser.add_option('-s', type='int', dest='size',
                      default=600, help='word vector size')
    ...
    try:
        word2vec_train(infile, outmodel, outvec,
