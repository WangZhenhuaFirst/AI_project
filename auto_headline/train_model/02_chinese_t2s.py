...
def zh_wiki_t2s(infile, outfile):
    ...
    for i, line in zip(range(len(t_corpus)), t_corpus):
        if i % 1000 == 0:
            logger.info(
                'convert t2s with the {}/{} line'.format(i, len(t_corpus)))
...
def t2s_main():
    ...
    parser.add_option('-i', '--input', dest='input_file',
                      default='01_corpus.zhwiki.txt', help='traditional file')
    # outfile为输出处理好的繁体转简体文件名称，默认corpus.zhwiki.simplified.txt
    parser.add_option('-o', '--output', dest='output_file',
                      default='02_corpus.zhwiki.simplified.txt', help='simplified file')
